<div class="row" style="min-height:100vh;">
  <div class="col-6-xl col-12-xs">
    <button class="button button-link button-icon"> <svg xmlns="http://www.w3.org/2000/svg" style="width: 24px; height:24px;" viewBox="0 0 20 20" fill="currentColor">
        <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
      </svg>
      </svg> Retour
    </button>
    <div class="container-center">
      <div class="w-80 mt-4">
        <img class="logo mx-6" src="./assets/images/BMW_logo_(gray).svg.png" alt="logo">
        <h1 class="ml-4">S'inscrire</h1>
        <h3 class="mb-8 ml-8">Pour accéder à cette page il est nécessaire de vous créer un compte</h3>

        <div class="pl-5">

          <div class="column pr-3 pl-3 pb-5">
            <label class="mb-2">Prénom</label>
            <input type="text" class="input w-100" placeholder="">
          </div>

          <div class="column pr-3 pl-3 pb-5">
            <label class="mb-2">Nom</label>
            <input type="text" class="input w-100" placeholder="">
          </div>

          <div class="column pr-3 pl-3 pb-5">
            <label class="mb-2">Email</label>
            <input type="text" class="input w-100" placeholder="mail@exemple.com">
          </div>


          <div class="column pr-3 pl-3 pb-5">
            <label class="mb-2">Mot de passe</label>
            <input type="password" class="input w-100">
          </div>

          <div class="column pr-3 pl-3">
            <label class="mb-2">Confirmez votre mot de passe</label>
            <input type="password" class="input w-100">
          </div>

          <div class="column mt-8 pr-3 pl-3 pb-5">
            <button class="button button-primary">S'inscrire</button>
            <button class="button button-secondary" onclick="location.href='/login'">Vous avez déjà un compte ? Connectez-vous</button>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div class="col-6-xl hidden-under-xl">
    <img class="fit-image" src="./assets/images/dmitry-timofeew-UU18rjWiQmo-unsplash.jpg" alt="side-image">
  </div>
</div>